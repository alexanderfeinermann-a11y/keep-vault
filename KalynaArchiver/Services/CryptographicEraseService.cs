...
public sealed class CryptographicEraseService
{
    ...
    public async Task<CryptoEraseAnalysis> AnalyzeAsync(string path, CancellationToken cancellationToken)
    {
        ...
        string message = encrypted
            ? "Encrypted v7 container detected. Cryptographic erase can delete the encrypted container, but hardware per-file secure erase is not guaranteed on SSDs."
            : "This is not an encrypted v7 container. Plain files cannot be cryptographically erased by deleting a key.";
