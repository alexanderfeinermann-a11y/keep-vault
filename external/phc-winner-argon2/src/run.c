...
int main(int argc, char *argv[]) {
    ...
    /*
     * The command-line utility is also used for raw binary test vectors.
     * Windows text-mode stdin translates newline bytes, so select binary mode
     * before reading an Argon2 password/prehash from a redirected pipe.
     */
#if defined(_WIN32)
    if (_setmode(_fileno(stdin), _O_BINARY) == -1) {
        fatal("could not set stdin to binary mode");
    }
#endif
